...
func DoClientHandshake(log logrus.FieldLogger, conn net.Conn,
	cHello ClientHello) (TUNIP, TUNGateway, serverTUNIP, serverTUNGateway net.IP, err error) {
	log.Debugf("Sending client hello: %v", cHello)

	if err := WriteJSON(conn, &cHello); err != nil {
		return nil, nil, nil, nil, fmt.Errorf("error sending client hello: %w", err)
	}

	var sHello ServerHello
	if err := ReadJSON(conn, &sHello); err != nil {
		return nil, nil, nil, nil, fmt.Errorf("error reading server hello: %w", err)
	}

	log.Debugf("Got server hello: %v", sHello)

	if sHello.Status != HandshakeStatusOK {
		return nil, nil, nil, nil, fmt.Errorf("got status %d (%s) from the server", sHello.Status, sHello.Status)
	}

	return sHello.TUNIP, sHello.TUNGateway, sHello.ServerTUNIP, sHello.ServerTUNGateway, nil
}
